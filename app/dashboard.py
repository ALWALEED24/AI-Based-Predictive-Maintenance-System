...
st.markdown(
    ...
    button, [role="button"], .stButton > button, .stDownloadButton > button, select, input, textarea {
...
def status_display_and_class(alert_level):
    ...
    if alert_level in ["Low Risk", "Medium Risk"]:
...
def prepare_machine_table_rows(df, max_rows=None, balanced=False):
    ...
    if balanced:
        ...
        if "health_score" in table_df.columns:
            table_df = table_df.sort_values("health_score", ascending=True)
...
def render_interactive_machine_table(df, title="Machines Status", max_rows=12, balanced=False, show_view_all=False):
    ...
    h0, h1, h2, h3, h4, h5, h6, h7 = st.columns([0.45, 1.25, 1.25, 1.45, 1.1, 1.9, 0.9, 0.8])
    with h0:
    ...
    with h6:
        st.markdown('<div class="machine-header-card">Update</div>', unsafe_allow_html=True)
    with h7:
    ...
    for idx, row in table_df.reset_index(drop=True).iterrows():
        ...
        c0, c1, c2, c3, c4, c5, c6, c7 = st.columns([0.45, 1.25, 1.25, 1.45, 1.1, 1.9, 0.9, 0.8])
        ...
        with c6:
            st.markdown('<div class="machine-row-card"><div class="machine-cell-muted">Latest</div></div>', unsafe_allow_html=True)
        with c7:
...
def create_machine_report(record):
    ...
    def split_probable_cause(cause_text):
        """
        Split probable cause into:
        - main cause
        - key observed condition
        """
        cause_text = get_value("probable_cause")

        if "Key observed condition:" in cause_text:
            main_cause, key_condition = cause_text.split("Key observed condition:", 1)
            main_cause = main_cause.strip()
            key_condition = key_condition.strip()
            return main_cause, key_condition

        return cause_text.strip(), get_value("short_reason")

    def clean_recommended_action(action_text):
        """
        Avoid duplicated wording such as:
        Pump - Critical: Critical pump condition. Recommended action: ...
        """
        action_text = str(action_text)

        if "Recommended action:" in action_text:
            return action_text.split("Recommended action:", 1)[1].strip()

        return action_text.strip()

    def build_ai_explanation():
        equipment = get_value("equipment_category")
        health_score = get_value("health_score")
        alert_level = get_value("alert_level")
        anomaly_label = get_value("if_anomaly_label")
        problem = get_value("problem_detected")
        solution = clean_recommended_action(get_value("recommended_action"))

        main_cause, key_condition = split_probable_cause(get_value("probable_cause"))

        if str(anomaly_label) == "1":
            model_sentence = (
                "The Isolation Forest model classified this record as abnormal because "
                "the sensor pattern is different from the normal operating pattern."
            )
        else:
            model_sentence = (
                "The Isolation Forest model classified this record as normal, but the "
                "health score is still used to monitor the machine condition."
            )

        return (
            f"- {model_sentence} The machine is a {equipment} with a health score of {health_score}, "
            f"classified as {alert_level}. Possible problem: {problem}.\n"
            f"- Probable cause: {main_cause}.\n"
            f"- Key observed condition: {key_condition}.\n"
            f"- The main abnormal sensor patterns are: {key_condition}. "
...
--------------
- {main_cause}.
- Key observed condition: {key_condition}.
...
Schedule Status     : {schedule["schedule_status"]}
Suggested Due Date  : {schedule["suggested_due_date"]} ({schedule["due_text"]})
Scheduling Reason   : {schedule["reason"]}
...
with st.sidebar:
    ...
    equipment_options = ["All"] + sorted(machine_df["equipment_category"].dropna().unique().tolist())
    selected_equipment = st.selectbox("Equipment Category", equipment_options)

    equipment_filtered_df = machine_df.copy()
    if selected_equipment != "All":
        equipment_filtered_df = equipment_filtered_df[
            equipment_filtered_df["equipment_category"] == selected_equipment
        ]

    alert_options = ["All"] + sorted(equipment_filtered_df["alert_level"].dropna().unique().tolist())
    ...
    if selected_alert != "All":
        alert_filtered_df = alert_filtered_df[
            alert_filtered_df["alert_level"] == selected_alert
        ]

    machine_options = ["All"] + sorted(alert_filtered_df["machine_id"].dropna().unique().tolist())
    selected_machine = st.selectbox("Machine ID", machine_options)
...
if page == "Dashboard":
    ...
    healthy_count = int((filtered_df["alert_level"] == "Normal").sum()) if not filtered_df.empty else 0
    warning_count = int(filtered_df["alert_level"].isin(["Low Risk", "Medium Risk"]).sum()) if not filtered_df.empty else 0
    critical_count = int(filtered_df["alert_level"].isin(["High Risk", "Critical"]).sum()) if not filtered_df.empty else 0

    cols = st.columns(4)
    kpi_card(ICONS["database"], "Total Machines", total_machines, cols[0], f"Across {category_count} categories", "blue")
    kpi_card(ICONS["check_circle"], "Healthy", healthy_count, cols[1], "Normal condition records", "green")
    kpi_card(ICONS["alert_triangle"], "Warning", warning_count, cols[2], "Low and medium risk records", "yellow")
    kpi_card(ICONS["zap"], "Critical", critical_count, cols[3], "High and critical risk records", "red")
    ...
    if filtered_df.empty:
        ...
        with col_left:
            ...
            fig_donut.add_annotation(
                text=f"<b>{total_records}</b><br><span style='font-size:12px;color:#9aa8bd'>Records</span>",
        ...
        with col_mid:
            ...
            fig_trend.update_layout(
                **PLOTLY_LAYOUT,
                title="Health Score Trend Across Recent Records",
...
# =========================================================
    ...
    if filtered_df.empty:
        ...
        render_machine_page_actions(filtered_df)
        st.markdown("<br>", unsafe_allow_html=True)
