...
MAINTENANCE_ALERT_LOG_PATH = os.path.join(BASE_DIR, "outputs", "maintenance_alert_log.csv")
...
@st.cache_data
def load_maintenance_alert_log():
    if not os.path.exists(MAINTENANCE_ALERT_LOG_PATH):
        return None
    return pd.read_csv(MAINTENANCE_ALERT_LOG_PATH)
...
def get_machine_level_status(df):
    """
    Convert record-level output into one final machine-level row per machine.

    Updated SmartMaint AI logic:
    - No forced alert distribution.
    - Alert levels come from the AI pipeline / decision-support logic.
    - Original sensor values are preserved for dashboard and report display.
    - If older output files do not contain alert levels, health score is used as a fallback.
    """
    ...
    if "alert_level" not in machine_df.columns:
        machine_df["alert_level"] = machine_df["health_score"].apply(classify_alert_level_from_health)
    else:
        machine_df["alert_level"] = machine_df["alert_level"].fillna(
            machine_df["health_score"].apply(classify_alert_level_from_health)
        )

    if "alert_priority" not in machine_df.columns:
        machine_df["alert_priority"] = machine_df["alert_level"].apply(classify_alert_priority_from_level)
    else:
        machine_df["alert_priority"] = machine_df["alert_priority"].fillna(
            machine_df["alert_level"].apply(classify_alert_priority_from_level)
        )
    ...
    for machine_id, group in machine_df.groupby("machine_id", sort=False):
        ...
        if "combined_ai_risk_score" in group.columns:
            combined_risk = pd.to_numeric(
                group["combined_ai_risk_score"],
                errors="coerce"
            ).fillna(0)
            group["_machine_risk_score"] = combined_risk
            representative_row = group.sort_values(
                by=["_machine_risk_score", "health_score"],
            ...
            group["_machine_risk_score"] = risk_score
            representative_row = group.sort_values("health_score", ascending=True).iloc[0].copy()

        # Use the average machine condition for the final machine-level health score.
        # The worst record is still used for the explanation row.
        representative_row["health_score"] = round(avg_health, 2)

        # Keep AI/decision-support alert if available. Otherwise classify using health score.
        if pd.isna(representative_row.get("alert_level", None)) or str(representative_row.get("alert_level", "")).strip() == "":
            representative_row["alert_level"] = classify_alert_level_from_health(representative_row["health_score"])

        representative_row["alert_priority"] = classify_alert_priority_from_level(representative_row["alert_level"])

        if representative_row["alert_level"] == "Normal":
            representative_row["if_anomaly_label"] = 0
            if str(representative_row.get("problem_detected", "")).strip() in ["", "nan", "None"]:
                representative_row["problem_detected"] = "No significant abnormality detected"
            if str(representative_row.get("recommended_action", "")).strip() in ["", "nan", "None"]:
                representative_row["recommended_action"] = "Continue normal operation and routine monitoring"
    ...
    machine_level_df["__risk_priority"] = (
        machine_level_df["alert_level"].astype(str).map(ALERT_RISK_PRIORITY).fillna(0)
    )
    machine_level_df["__health_numeric"] = pd.to_numeric(
        machine_level_df["health_score"],
        errors="coerce"
    ).fillna(100)
    ...
    machine_level_df = machine_level_df.sort_values(
        ...
        columns=["__risk_priority", "__health_numeric", "_machine_risk_score"],
        errors="ignore"
    )
...
def render_sensor_reading_cards(record, sensor_columns):
    ...
    label_map = {
        "Type": ("Machine Type", "Product type code"),
...
def build_alert_log_from_machine_view(machine_view):
    """Fallback alert log for older outputs when maintenance_alert_log.csv does not exist."""
    if machine_view is None or machine_view.empty:
        return pd.DataFrame()

    alert_df = machine_view.copy()
    ...
    alert_df["alert_id"] = [
        f"ALT-{idx + 1:04d}" for idx in range(len(alert_df))
    ]
    alert_df["alert_status"] = "Active"
    alert_df["generated_at"] = get_last_updated(MACHINE_RESULTS_PATH)

    rename_map = {
        "problem_detected": "problem",
        "short_reason": "observed_pattern",
    }
    alert_df = alert_df.rename(columns=rename_map)

    if "probable_cause" not in alert_df.columns:
        alert_df["probable_cause"] = "Generated from health score and anomaly evidence."

    required_cols = [
        "alert_id", "machine_id", "equipment_category", "alert_level",
        "alert_priority", "health_score", "problem", "probable_cause",
        "recommended_action", "observed_pattern", "alert_status",
        "generated_at"
    ]
    ...
    return alert_df[required_cols]


def filter_alert_log(alert_log_df, selected_equipment="All", selected_alert="All", selected_machine="All"):
    if alert_log_df is None or alert_log_df.empty:
        return pd.DataFrame()

    filtered_alerts = alert_log_df.copy()

    if selected_equipment != "All" and "equipment_category" in filtered_alerts.columns:
        filtered_alerts = filtered_alerts[
            filtered_alerts["equipment_category"].astype(str) == selected_equipment
        ]

    if selected_alert != "All" and "alert_level" in filtered_alerts.columns:
        filtered_alerts = filtered_alerts[
            filtered_alerts["alert_level"].astype(str) == selected_alert
        ]

    if selected_machine != "All" and "machine_id" in filtered_alerts.columns:
        filtered_alerts = filtered_alerts[
            filtered_alerts["machine_id"].astype(str) == selected_machine
        ]

    return filtered_alerts.reset_index(drop=True)
...
def get_page_from_query_params():
    valid_pages = ["Dashboard", "Machines", "Alert Log", "Reports"]
    try:
...
alert_log_df = load_maintenance_alert_log()
...
with st.sidebar:
    ...
    st.markdown('<div class="sidebar-section-caption">Choose the main workspace for monitoring, inspection, or reports.</div>', unsafe_allow_html=True)
    ...
    sidebar_nav_link("Maintenance Alert Log", "Track active maintenance alerts", ICONS["alert_triangle"], "Alert Log", current_page == "Alert Log")
    sidebar_nav_link("Reports", "Export summaries and maintenance reports", ICONS["file"], "Reports", current_page == "Reports")
    ...
    # ---------------------------------------------------------
    # IMPORTANT FIX:
    # Streamlit reruns the script every time a selectbox changes.
    # Using stable session_state keys prevents the sidebar filters from
    # returning to "All" after choosing equipment, alert level, or machine ID.
    equipment_options = ["All"] + sorted(
        filter_source_df["equipment_category"].dropna().astype(str).unique().tolist()
    )

    if "filter_equipment" not in st.session_state:
        st.session_state["filter_equipment"] = "All"

    if st.session_state["filter_equipment"] not in equipment_options:
        st.session_state["filter_equipment"] = "All"
    ...
    if selected_equipment != "All":
        equipment_filtered_df = equipment_filtered_df[
            equipment_filtered_df["equipment_category"].astype(str) == selected_equipment
        ]

    # Alert Level Filter
    # Show only alert levels that actually exist for the selected equipment category.
    # Example: if Agitator has no Normal machines, Normal will not appear in the dropdown.
    ...
    if "filter_alert" not in st.session_state:
        st.session_state["filter_alert"] = "All"

    # If the previous alert level does not exist for the newly selected
    # equipment type, keep the app stable by returning only this filter to All.
    if st.session_state["filter_alert"] not in alert_options:
        st.session_state["filter_alert"] = "All"
    ...
    if selected_alert != "All":
        alert_filtered_df = alert_filtered_df[
            alert_filtered_df["alert_level"].astype(str) == selected_alert
        ]

    # Machine ID Filter
    machine_options = ["All"] + sorted(
        alert_filtered_df["machine_id"].dropna().astype(str).unique().tolist()
    )

    if "filter_machine" not in st.session_state:
        st.session_state["filter_machine"] = "All"

    # If the previous machine is not available after equipment/alert filtering,
    # reset only Machine ID. Equipment Category and Alert Level remain selected.
    if st.session_state["filter_machine"] not in machine_options:
        st.session_state["filter_machine"] = "All"
    ...
    st.markdown(
        ...
        <div class="sidebar-tip">
            {ICONS["info"]}
            Use <b>Dashboard</b> for overview, <b>Machine Monitoring</b> for detailed inspection,
            <b>Maintenance Alert Log</b> for active alerts, and <b>Reports</b> to export summaries.
        </div>
        """,
        unsafe_allow_html=True
    )
...
# =========================================================
    ...
    st.markdown(f'<div class="section-title">{ICONS["alert_triangle"]} Maintenance Alert Log</div>', unsafe_allow_html=True)

    if alert_log_df is None or alert_log_df.empty:
        alert_log_source = build_alert_log_from_machine_view(machine_view_df)
        source_note = "maintenance_alert_log.csv was not found, so this page generated a temporary alert log from machine results."
    else:
        alert_log_source = alert_log_df.copy()
        source_note = "Loaded from outputs/maintenance_alert_log.csv."

    filtered_alert_log = filter_alert_log(
        alert_log_source,
    ...
    filtered_alert_log = filtered_alert_log.drop(
    ...
    st.markdown(
        ...
        <div class="info-box">
            {ICONS["info"]}
            <span>
                This page turns AI risk results into a practical maintenance alert log.
                Alerts can be used by technicians to inspect, acknowledge, resolve, or review machine problems.
                <br><b>Source:</b> {html.escape(source_note)}
            </span>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if filtered_alert_log.empty:
        st.success("No active maintenance alerts found for the selected filters.")
    else:
        critical_alerts = int((filtered_alert_log["alert_level"].astype(str) == "Critical").sum()) if "alert_level" in filtered_alert_log.columns else 0
        high_alerts = int((filtered_alert_log["alert_level"].astype(str) == "High Risk").sum()) if "alert_level" in filtered_alert_log.columns else 0
        medium_alerts = int((filtered_alert_log["alert_level"].astype(str) == "Medium Risk").sum()) if "alert_level" in filtered_alert_log.columns else 0
        active_alerts = int((filtered_alert_log.get("alert_status", "Active").astype(str) == "Active").sum()) if "alert_status" in filtered_alert_log.columns else len(filtered_alert_log)

        avg_alert_health = pd.to_numeric(
            filtered_alert_log["health_score"],
            errors="coerce"
        ).mean() if "health_score" in filtered_alert_log.columns else 0

        alert_cols = st.columns(5)
        kpi_card(ICONS["alert_triangle"], "Total Alerts", len(filtered_alert_log), alert_cols[0], "Filtered alert records", "blue")
        kpi_card(ICONS["zap"], "Active", active_alerts, alert_cols[1], "Requires review", "orange")
        kpi_card(ICONS["alert_triangle"], "Critical", critical_alerts, alert_cols[2], "Immediate action", "red")
        kpi_card(ICONS["zap"], "High Risk", high_alerts, alert_cols[3], "Urgent attention", "orange")
        kpi_card(ICONS["gauge"], "Avg Health", f"{avg_alert_health:.2f}", alert_cols[4], "Alert machines only", "yellow")

        st.markdown("<br>", unsafe_allow_html=True)

        st.download_button(
            label="Download Maintenance Alert Log CSV",
            data=filtered_alert_log.to_csv(index=False),
            file_name="smartmaint_ai_maintenance_alert_log.csv",
            mime="text/csv",
        ...
        with chart_left:
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.markdown(f'<div class="section-title">{ICONS["bar_chart"]} Alerts by Level</div>', unsafe_allow_html=True)

            alert_summary = filtered_alert_log["alert_level"].value_counts().reset_index()
            alert_summary.columns = ["alert_level", "count"]
            alert_summary["alert_level"] = pd.Categorical(
                alert_summary["alert_level"],
                categories=["Normal", "Low Risk", "Medium Risk", "High Risk", "Critical"],
                ordered=True
            )
            alert_summary = alert_summary.sort_values("alert_level")
            ...
            fig_alerts.update_layout(
                **PLOTLY_LAYOUT,
                title="Maintenance Alerts by Risk Level",
        ...
        with chart_right:
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.markdown(f'<div class="section-title">{ICONS["factory"]} Alerts by Equipment</div>', unsafe_allow_html=True)

            equipment_alerts = (
                filtered_alert_log.groupby("equipment_category")
                .size()
            ...
            fig_equipment_alerts.update_traces(
                textfont_color="#f4f7fb",
                marker_line_width=0,
                textposition="outside",
                opacity=0.92
            )
            fig_equipment_alerts.update_layout(
                **PLOTLY_LAYOUT,
                title="Top Equipment Categories with Alerts",
                xaxis_title="Number of Alerts",
                yaxis_title="Equipment Category",
                height=360,
                showlegend=False
            )
            fig_equipment_alerts.update_yaxes(autorange="reversed")
            st.plotly_chart(fig_equipment_alerts, use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)

        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        alert_columns = safe_column_list(
            filtered_alert_log,
            [
                "alert_id", "machine_id", "equipment_category", "alert_level",
                "alert_priority", "health_score", "problem", "probable_cause",
        ...
        render_pretty_dataframe_table(
            filtered_alert_log[alert_columns],
            title="Maintenance Alert Log Table",
            max_rows=40
        )
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown("---")
        st.markdown(f'<div class="section-title">{ICONS["tool"]} Maintenance Action Summary</div>', unsafe_allow_html=True)

        action_summary = filtered_alert_log.groupby(
            ["alert_level", "recommended_action"]
        ).size().reset_index(name="count")

        render_pretty_dataframe_table(action_summary, title="Recommended Actions by Alert Level", max_rows=20)
...
# =========================================================
    ...
    if filtered_df.empty:
        ...
        report_total_machines = filtered_df["machine_id"].nunique()
        report_avg_health = filtered_df["health_score"].mean()
        report_critical_count = int((filtered_df["alert_level"] == "Critical").sum())
        report_high_count = int((filtered_df["alert_level"] == "High Risk").sum())

        report_cols = st.columns(4)
        kpi_card(ICONS["database"], "Report Machines", report_total_machines, report_cols[0], "Filtered machine count", "blue")
        kpi_card(ICONS["gauge"], "Average Health", f"{report_avg_health:.2f}", report_cols[1], "Filtered average score", "green")
        kpi_card(ICONS["alert_triangle"], "High Risk", report_high_count, report_cols[2], "Urgent attention", "orange")
        kpi_card(ICONS["alert_triangle"], "Critical", report_critical_count, report_cols[3], "Immediate action", "red")
        ...
        with c1:
            st.markdown(
                ...
                <div class="app-card">
                    ...
                    <div class="app-card-text">Includes all visible machines based on the current filters.</div>
                </div>
                """,
                unsafe_allow_html=True
            )
...
System Summary
--------------
Total Machines      : {filtered_df["machine_id"].nunique()}
...
"""
            st.markdown(
                ...
                <div class="app-card">
                    ...
                    <div class="app-card-value">Maintenance Summary</div>
            ...
            st.download_button(
                label="Download Maintenance Summary",
                data=summary_text,
        ...
        with chart_left:
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.markdown(f'<div class="section-title">{ICONS["bar_chart"]} Alert Level Summary</div>', unsafe_allow_html=True)

            alert_summary = filtered_df["alert_level"].value_counts().reset_index()
            alert_summary.columns = ["alert_level", "count"]
            alert_summary["alert_level"] = pd.Categorical(
                alert_summary["alert_level"],
                categories=["Normal", "Low Risk", "Medium Risk", "High Risk", "Critical"],
            ...
            fig_alert_report.update_traces(
                textfont_color="#f4f7fb",
                marker_line_width=0,
                textposition="outside",
                opacity=0.92
            )
            fig_alert_report.update_layout(
                **PLOTLY_LAYOUT,
                title="Machines by Alert Level",
                xaxis_title="Alert Level",
                yaxis_title="Number of Machines",
                showlegend=False,
                height=360
            )
            st.plotly_chart(fig_alert_report, use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)

        with chart_right:
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.markdown(f'<div class="section-title">{ICONS["alert_triangle"]} Top High-Risk Machines</div>', unsafe_allow_html=True)

            top_risk_df = filtered_df.sort_values("health_score", ascending=True).head(10)
            ...
            fig_top_risk = px.bar(
                ...
                orientation="h",
                color_discrete_map=ALERT_COLOR_MAP,
                hover_data=["equipment_category", "alert_level"]
            )
            fig_top_risk.update_traces(marker_line_width=0, opacity=0.92)
            fig_top_risk.update_layout(
                **PLOTLY_LAYOUT,
                title="Lowest Health Score Machines",
                xaxis_title="Health Score",
                yaxis_title="Machine ID",
                height=360
            )
            fig_top_risk.update_yaxes(
                autorange="reversed",
                gridcolor="rgba(59,83,122,0.24)"
            )
            st.plotly_chart(fig_top_risk, use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)

        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.markdown(f'<div class="section-title">{ICONS["factory"]} Equipment Category Risk Summary</div>', unsafe_allow_html=True)

        risk_levels = ["Medium Risk", "High Risk", "Critical"]
        equipment_risk_summary = filtered_df.copy()
        equipment_risk_summary["is_risky"] = equipment_risk_summary["alert_level"].isin(risk_levels)

        equipment_chart = (
            equipment_risk_summary
            .groupby("equipment_category")
            .agg(
                total_machines=("machine_id", "count"),
                risky_machines=("is_risky", "sum"),
                average_health=("health_score", "mean")
            )
            .reset_index()
            .sort_values(["risky_machines", "average_health"], ascending=[False, True])
            .head(12)
        )

        fig_equipment_risk = px.bar(
            equipment_chart,
            x="equipment_category",
            y="risky_machines",
            text="risky_machines",
            color="average_health",
            color_continuous_scale="Blues",
            hover_data=["total_machines", "average_health"]
        )
        fig_equipment_risk.update_traces(
            textfont_color="#f4f7fb",
            marker_line_width=0,
            textposition="outside",
            opacity=0.92
        )
        fig_equipment_risk.update_layout(
            **PLOTLY_LAYOUT,
            title="Risky Machines by Equipment Category",
            xaxis_title="Equipment Category",
            yaxis_title="Risky Machines",
            height=390,
            showlegend=False
        )
        st.plotly_chart(fig_equipment_risk, use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
