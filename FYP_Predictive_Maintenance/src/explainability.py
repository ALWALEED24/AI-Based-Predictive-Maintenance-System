...
def generate_explanation(row):
    """
    Generate short rule-based AI monitoring explanation without repeating problem/cause/solution.
    """
    ...
    if row["if_anomaly_label"] == 1:
        ...
        model_part = (
            "The AI monitoring system compared this machine record with learned operational patterns. "
            "No strong anomaly was detected, but the health score is still used as an operational risk indicator for condition monitoring."
        )

    return (
        f"{model_part} Based on the health score and detected sensor behavior, "
        f"the machine was classified as {alert_level}. "
        "This result is intended to support condition-based maintenance decision-making, "
        "but the final maintenance action should still be confirmed by an engineer or technician."
    )
